...
export default async function ContenidoPage() {
  ...
  return (
    <div>
      <p className="text-xs font-mono uppercase tracking-widest text-[#7bc96f] mb-2">Biblioteca</p>
      <h1 className="text-3xl font-light font-serif italic mb-8">Contenido exclusivo</h1>

      {!items?.length ? (
        <div className="border border-white/10 rounded-lg p-8 text-center">
          <p className="text-gray-400 text-sm">Próximamente — estamos preparando contenido exclusivo para miembros.</p>
        </div>
      ) : (
        <div className="grid gap-4">
          {items.map(item => (
            <div key={item.id} className="border border-white/10 rounded-lg p-6 hover:border-white/20 transition-colors">
